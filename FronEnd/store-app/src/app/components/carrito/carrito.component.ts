...
export class CarritoComponent implements OnInit {
  ...
  obtenerCarrito(): void {    
    ...
    this.apiService.obtenerCarrito().subscribe(
      ...
      (error) => {
        this.mensajeError = 'Error al obtener el carrito.';
      }
    );
  }

  agregarArticulo(articuloId: number): void {
    this.apiService.agregarAlCarrito(articuloId).subscribe(() => {
      this.obtenerCarrito();
    });
  }

  quitarArticulo(articuloId: number): void {    
    this.apiService.eliminarArticulo(articuloId).subscribe(() => {
      this.obtenerCarrito();
    });
  }

  eliminarArticulo(articuloId: number): void {
    this.apiService.eliminarTodosArticulo(articuloId).subscribe(() => {
      this.obtenerCarrito();
    });
  }
